...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        // Start our defined Container first Engine
        if (container != null) {
        ...
        // 正常配置下 executors 应该为空集合，debug 时注意
        synchronized (executors) {
            for (Executor executor: executors) {
                executor.start();
            }
        ...
        // 先启动 Service 的 child container, 再启动 mapperListener, 在启动 Host 时已经对 Context 和  Wapper 初始化过一次, 在调用 mapperListener 的 startInternal
        // 方法时会递归 addListenter 和 registerHost (同时处理 Context 和 Wapper),后续变化通过观察者模式更新
